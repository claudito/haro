...
if (isset($_POST['alm']) AND isset($_POST['tipo_transaccion']) AND isset($_POST['usuario']) AND isset($_POST['fecha_inicio'])  AND isset($_POST['comentario']) AND isset($_POST['codigo_cc']) AND isset($_POST['area']) AND isset($_POST['tipo']) AND isset($_POST['moneda'])) 
{
...
if (strlen($alm)>0 AND strlen($tran)>0 AND strlen($fecha_inicio)>0 AND strlen($comentario)>0 AND strlen($centro_costo)>0  AND strlen($area)>0 AND strlen($tipo)>0 AND strlen($moneda)>0) 
{
  
$objeto      =  new Movalmcab($numero,$alm,$tran,$id_usuario,$fecha_inicio,$comentario,$centro_costo,$area,$tipo,$moneda);

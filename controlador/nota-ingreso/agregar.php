...
if (isset($_POST['alm']) AND isset($_POST['tipo_transaccion']) AND isset($_POST['tipo_documento'])  AND isset($_POST['usuario']) AND isset($_POST['proveedor']) AND isset($_POST['fecha_inicio'])  AND isset($_POST['comentario']) AND isset($_POST['codigo_cc']) AND isset($_POST['area']) AND isset($_POST['tipo']) AND isset($_POST['moneda'])) 
{
	...
	$doc_oc 			=  $funciones->validar_xss($_POST['doc_oc']);
...
if (strlen($alm)>0 AND strlen($tran)>0 AND strlen($doc_ref)>0 AND strlen($id_usuario)>0 AND strlen($id_proveedor)>0 AND strlen($fecha_inicio)>0 AND strlen($comentario)>0 AND strlen($centro_costo)>0  AND strlen($area)>0 AND strlen($tipo)>0 AND strlen($moneda)>0) 
{
  
$objeto      =  new Movalmcab_salida($numero,$alm,$tran,$doc_ref,$num_ref,$doc_oc,$id_usuario,$id_proveedor,$fecha_inicio,$fecha_fin,$comentario,$centro_costo,$ot,$area,$tipo,$estado,$moneda);

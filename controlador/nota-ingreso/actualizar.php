...
if (isset($_POST['id']) AND isset($_POST['tipo_transaccion']) AND isset($_POST['tipo_documento']) AND isset($_POST['num_ref']) AND isset($_POST['proveedor']) AND isset($_POST['fecha_inicio']) AND isset($_POST['comentario']) AND isset($_POST['codigo_cc']) AND isset($_POST['area']) AND isset($_POST['tipo']) AND isset($_POST['estado']))
{
...
if (strlen($id)>0 AND strlen($tran)>0 AND strlen($doc_ref)>0 AND strlen($num_ref)>0 AND strlen($id_proveedor)>0 AND strlen($fecha_inicio)>0  AND strlen($comentario)>0 AND strlen($centro_costo)>0  AND strlen($area)>0 AND strlen($tipo)>0 AND strlen($estado)>0) 
{
	
$objeto      =  new Movalmcab_salida('','',$tran,$doc_ref,$num_ref,'','',$id_proveedor,$fecha_inicio,'',$comentario,$centro_costo,$ot,$area,$tipo,$estado,$moneda);
